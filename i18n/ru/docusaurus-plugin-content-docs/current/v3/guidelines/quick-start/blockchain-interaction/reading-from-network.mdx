...
The result of calling a get method from the TON HTTP API comes in _stack_ format and may be deserialized one by one using `readNumber()` or a similar function.
